...
void EnvironmentSensorManager::loop() {
  ...
  static long next_battery_update = 0;
  ...
  #if ENV_INCLUDE_INA219
  // Update board battery voltage from INA219 readings periodically
  if (INA219_initialized && millis() > next_battery_update) {
    float voltage_v = INA219.getBusVoltage_V();
    uint16_t voltage_mv = (uint16_t)(voltage_v * 1000);

    // Try to update the board's battery voltage if possible (Station G2 specific)
    #ifdef STATION_G2
    extern StationG2Board board;
    board.setBatteryVoltage(voltage_mv);
    #endif

    next_battery_update = millis() + 60000; // Update every minute
  }
  #endif
